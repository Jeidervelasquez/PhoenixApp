                ...
                with st.expander("📝 1. REGISTRAR RESULTADO"):
                    with st.form("form_scrim"):
                        ...
                        if st.form_submit_button("GUARDAR REGISTRO"):
                            ...
                            enviar_notificacion_tg(msg)
                            st.success("✅ Partida registrada en la base de datos."); st.rerun()
            with col_img:
                    ...
                    if ps:
                        ...
                        if st.button("VINCULAR IMAGEN"):
                            if img_file and id_partida:
                                ...
                                st.success("✅ Captura guardada para este escuadrón."); st.rerun()
                            else: st.warning("⚠️ Debes seleccionar un rival y subir un archivo primero.")
...
st.header("⏰ CONFIGURAR RECORDATORIOS")
# Creamos tres espacios para las horas
col1, col2, col3 = st.columns(3)

with col1:
    h1 = st.text_input("Mañana (Ej: 09:00)", value="09:00")
with col2:
    h2 = st.text_input("Tarde (Ej: 16:00)", value="16:00")
with col3:
    h3 = st.text_input("Noche (Ej: 20:00)", value="20:00")

if st.button("GUARDAR HORARIOS EN EL CLAN"):
    # Guardamos las horas en una nueva carpeta de Firebase llamada 'configuracion'
    nuevos_horarios = {"h1": h1, "h2": h2, "h3": h3}
    db.reference('configuracion/horarios').set(nuevos_horarios)
    st.success("✅ Horarios actualizados. El bot los tomará en unos minutos.")

# Definimos quiénes tienen permiso
rangos_autorizados = ["Administrador", "Moderador", "Líder"]

# Buscamos el rol del usuario actual en tu base de datos
# (Asegúrate de que 'datos_usuario' sea donde guardas la info del que inició sesión)
rol_usuario = datos_usuario.get("rol", "Usuario") 

if rol_usuario in rangos_autorizados:
    st.divider()
    st.header("👑 PANEL DE CONTROL DE ALARMAS")
    st.subheader("Configuración exclusiva para Rangos Altos")
    ...
    if st.button("ACTUALIZAR HORARIOS DEL CLAN"):
        # Guardamos en la carpeta 'configuracion' de Firebase
        nuevos_horarios = {"h1": h1, "h2": h2, "h3": h3}
        db.reference('configuracion/horarios').set(nuevos_horarios)
        st.success(f"✅ ¡Entendido {rol_usuario}! Horarios actualizados en Firebase.")
else:
    # Esto es lo que verá un usuario normal (un Soldado o Aspirante)
    st.info("ℹ️ Los recordatorios automáticos están activos, pero solo los Administradores pueden cambiar las horas.")
